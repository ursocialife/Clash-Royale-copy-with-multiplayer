...
class MultiplayerService {
    ...
    private async cleanup() {
        ...
        this.myCode = null;
        this.isHost = false;
        // Increased delay to ensure sockets close and IDs are freed
    ...
    public async joinGame(code: string, onConnect: () => void, onError: (err: string) => void) {
        ...
        try {
            ...
            this.peer.on('open', () => {
                ...
                connectionTimeout = setTimeout(() => { if (!this.conn || !this.conn.open) { onError("Connection timed out."); this.disconnect(); } }, 15000); // Increased timeout
            });
